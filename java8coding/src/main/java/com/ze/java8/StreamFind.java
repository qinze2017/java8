...
public class StreamFind {
    ...
    public static void main(String[] args) {
        ...
        Optional<Integer> optional2 = stream.filter(i -> i > 10).findFirst();
        System.out.println(optional2.orElse(-1));

        stream = Arrays.stream(new Integer[]{1, 2, 3, 4, 5, 6, 7});
        Optional<Integer> optional3 = stream.filter(i -> i % 2 == 0).findFirst();
        optional3.ifPresent(System.out::println);

        System.out.println(optional3.filter(i -> i == 2).get());
